...
1)
    echo -e "${YELLOW}->>开始执行nginx部署命令。。。${RES}"
    {
        ...
        docker run --name nginx-test -v ~/data/nginx/conf/nginx.conf:/etc/nginx/nginx.conf -v ~/data/nginx/log:/var/log/nginx -v ~/data/nginx/html:/usr/share/nginx/html nginx
    }
;;
2)
